...
export function ScheduleHistoryList() {
    ...
    if (error) {
        return <p className="p-4 text-red-500">Erro ao carregar dados.</p>
    }
    ...
    const filtered = data.scheduleHistories.filter((h: any) => {
        ...
        const matchesDate = dateFilter ? new Date(h.updatedAt).toLocaleDateString("pt-BR") === dateFilter : true;
                ...
                {filtered.map((history: any) => (
                    <Card key={history.id}>
                        ...
                        <CardContent className="text-sm space-y-1">
                            <p><strong>Data do agendamento:</strong> {new Date(history.schedule.scheduledDate).toLocaleDateString("pt-BR")}</p>
                            <p><strong>Realizado em:</strong> {new Date(history.updatedAt).toLocaleDateString("pt-BR")}</p>
